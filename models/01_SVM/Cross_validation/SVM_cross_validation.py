# python3 SVM_cross_validation.py > log_cv.txt 2> err_cv.txt & 
# DURACIÓN COMPLETA: 

# Se utiliza la validación cruzada en 5 particiones para obtener unas estadísticas más fiables del rendimiento del modelo SVM.
...
np.random.seed(42)
print('DESCRIPTION: Support Vector Machine training, with cross validation to obtain more robust statistics.')
...
for fold in [1,2,3,4,5]:
    ...
    svm = LinearSVC(dual=False, C=0.01)
    
    svm.fit(X_train, y_train)
    print("Model fitted")

    # AttributeError: 'LinearSVC' object has no attribute 'predict_proba'
    # SVC supports probabilities, but it is even slower...
    # y_proba = svm.predict_proba(X_test)
    y_pred = svm.predict(X_test)

    test_acc[1].append(metrics.accuracy_score(y_test, y_pred))
    test_acc['MER'].append(MER(y_test.astype(str), y_pred.astype(str)))
