# python3 CNN_MNIST_test_e14.py > log_test.txt 2> err_test.txt & 
# DURACIÓN COMPLETA: 

# Se utiliza el modelo más apropiado elegido tras la cv, para entrenar con la base de datos aumentada.

# Se entrena en el corpus de training original, y se evalua en el test set.
...
print('DESCRIPTION: CNN training, using the original training set. Validation on test set. 14 epochs, on all the dataset. Model: MNIST from the book "Hands on Machine Learning" (Géron).')
...
checkpoint = tf.keras.callbacks.ModelCheckpoint(f"./models/best_model.h5", 
...
df.to_csv(f'./models/history.csv')

# Plot
df.plot(figsize=(8, 5))
plt.grid(True)
#plt.gca().set_ylim(0, 1)
plt.savefig(f"./plots/learning_curves_plot.png", dpi=200)
...
df.describe().to_csv('data_file_stats.csv')
