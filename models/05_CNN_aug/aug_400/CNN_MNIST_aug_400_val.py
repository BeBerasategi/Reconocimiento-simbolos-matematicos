# python3 CNN_MNIST_aug_400_val.py > log_aug.txt 2> err_aug.txt & 
# DURACIÓN COMPLETA: 

# Se utiliza el modelo más apropiado elegido tras la cv, para entrenar con la base de datos aumentada. Se reserva un set de validación para adaptar mejor la cantidad de epochs.
...
print('DESCRIPTION: CNN training, with data augmentation. Validation is used. Augmented dataset. Model: MNIST from the book "Hands on Machine Learning" (Géron).')
...
X_train= np.loadtxt("/home/jovyan/work/databases/Data_Augmentation/DA_400/X_aug.csv", delimiter=",")
y_train_id= np.loadtxt("/home/jovyan/work/databases/Data_Augmentation/DA_400/y_aug_id.csv", delimiter=",", dtype=str)
...
history = model.fit(X_train0, y_train0, epochs=50, callbacks=[early_stopping_cb, checkpoint],
                    verbose=2,
