# python3 CNN_MNIST_aug_e14.py > log_aug.txt 2> err_aug.txt & 
# DURACIÓN COMPLETA: 

# Se utiliza el modelo más apropiado elegido tras la cv, para entrenar con la base de datos aumentada.
...
print('DESCRIPTION: CNN training, with data augmentation. 14 epochs, on all the dataset. Model: MNIST from the book "Hands on Machine Learning" (Géron).')
print('Start time:', time.strftime("%H:%M:%S", time.gmtime(start_time)), end='\n\n')

X_train, y_train, symbol_id2index_train, index2symbol_id_train = load_database(f'/home/jovyan/work/databases/HASY/benat-data/test.csv')
...
# Load augmented data (~1min needed)
X_aug= np.loadtxt("/home/jovyan/work/databases/Data_Augmentation/X_aug.csv", delimiter=",")
y_aug_id= np.loadtxt("/home/jovyan/work/databases/Data_Augmentation/y_aug_id.csv", delimiter=",", dtype=str)
...
# Establecemos epochs=14 después de haber visto que de media funciona mejor (en cross-validation).
history = model.fit(X_train, y_train, epochs=14, callbacks=[early_stopping_cb, checkpoint],
